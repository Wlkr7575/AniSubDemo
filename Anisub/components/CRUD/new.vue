      ...
      <v-container>
        <v-form>
          ...
          <v-layout row justify-space-between>
            ...
            <v-btn class="ma-2" :color="`${$vuetify.theme.dark?'error':'primary'}`" :loading="actionLoading" depressed @click="upload(postid)">
              {{ check? 'Update': 'Upload' }}
            </v-btn>
          </v-layout>
        </v-form>
...
export default {
  ...
  computed: {
    ...
    date () {
      return this.$options.propsData.postid
    }
  },
  beforeCreate () {
    const _id = this.$options.propsData.postid
    Object.assign(this.$options.computed, { ...mapState({ update: state => state.store.data.find(p => p._id === _id) }) })
  },
  mounted () {
    if (this.update) {
